  ...
  try {
    ...
    if (product) {
      ...
      if (product.image_path) {
        const imagePath = new URL(product.image_path).pathname.split('/').pop();
        if (imagePath) {
      ...
      if (product.thumbnail_path) {
        const thumbnailPath = new URL(product.thumbnail_path).pathname
          .split('/')
          .pop();
        if (thumbnailPath) {
    ...
    if (data.image) {
      ...
      if (currentProduct) {
        if (currentProduct.image_path) {
          const imagePath = new URL(currentProduct.image_path).pathname
            .split('/')
            .pop();
          if (imagePath) {
            await supabase.storage.from('products').remove([imagePath]);
        ...
        if (currentProduct.thumbnail_path) {
          const thumbnailPath = new URL(currentProduct.thumbnail_path).pathname
            .split('/')
            .pop();
          if (thumbnailPath) {
            await supabase.storage.from('products').remove([thumbnailPath]);
